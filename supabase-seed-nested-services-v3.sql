-- Seed simplified nested services structure with 3 mutually exclusive options
-- Run this AFTER running supabase-migration-nested-services.sql
-- This will create Regular Cleaning, Deep Cleaning, and End of Tenancy as separate options

-- IMPORTANT: Clear existing services first
TRUNCATE services CASCADE;

-- Insert main service categories (mutually exclusive)
...
VALUES
  ...
  -- Main Category 2: Deep Cleaning
  (gen_random_uuid(), 'Deep Cleaning', 0, 0, true, 2, 'plus_minus', true, 'deep_cleaning', 0),

  -- Main Category 3: End of Tenancy Cleaning
  (gen_random_uuid(), 'End of Tenancy Cleaning', 0, 0, true, 3, 'plus_minus', true, 'end_of_tenancy', 0)
...
WITH deep_cleaning_cat AS (
  SELECT id FROM services WHERE name = 'Deep Cleaning' AND is_category = true LIMIT 1
)
INSERT INTO services (id, name, price, time_minutes, active, order_index, question_type, is_category, category_type, nesting_level, parent_id)
SELECT
  gen_random_uuid(),
...
-- End of Tenancy Cleaning Sub-services (Same as Deep Cleaning)
WITH eot_cat AS (
  SELECT id FROM services WHERE name = 'End of Tenancy Cleaning' AND is_category = true LIMIT 1
)
INSERT INTO services (id, name, price, time_minutes, active, order_index, question_type, is_category, category_type, nesting_level, parent_id)
SELECT
  gen_random_uuid(),
...
-- End of Tenancy Cleaning Extras
WITH eot_cat AS (
  SELECT id FROM services WHERE name = 'End of Tenancy Cleaning' AND is_category = true LIMIT 1
)
INSERT INTO services (id, name, price, time_minutes, active, order_index, question_type, is_category, category_type, nesting_level, parent_id)
...
INSERT INTO services (id, name, price, time_minutes, active, order_index, question_type, is_category, category_type, nesting_level)
VALUES
  (gen_random_uuid(), 'Gardening', 0, 0, true, 5, 'plus_minus', true, 'gardening', 0)
ON CONFLICT DO NOTHING;
